...
if [ "$1" == "bootimage" ]; then
    make -f device/bananapi/$TARGET_PRODUCT/Kernel.mk -j6 bootimage-quick
fi

if [ "$1" == "recoveryimage" ]; then
    make -f device/bananapi/$TARGET_PRODUCT/Kernel.mk -j6 recoveryimage-quick
fi

if [ "$1" == "menuconfig" ]; then
    make -f device/bananapi/$TARGET_PRODUCT/Kernel.mk kernelconfig
fi

if [ "$1" == "savedefconfig" ]; then
    make -i -f device/bananapi/$TARGET_PRODUCT/Kernel.mk savekernelconfig
fi

if [ "$1" == "build-modules" ]; then
	    make -f device/bananapi/$TARGET_PRODUCT/Kernel.mk -j8 build-modules-quick
fi
