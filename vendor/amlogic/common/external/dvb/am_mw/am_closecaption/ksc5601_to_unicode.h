...
int ksc5601_to_unicode (unsigned char ts_char1, unsigned char ts_char2)
{
    int i;
    int unicode_data = ts_char2;

    unsigned char c1 = ts_char1 - 0x80; //0x80+0x21
    unsigned char c2 = ts_char2 - 0x80;
    
    if ((c1 >= 0x21 && c1 <= 0x2c) || (c1 >= 0x30 && c1 <= 0x48) || (c1 >= 0x4a && c1 <= 0x7d)) 
    {
            if (c2 >= 0x21 && c2 < 0x7f) 
            {
                i = (c1 -0x21)*94 + (c2-0x21);
                //printf("...c1 is:0x%x, c2 is:0x%x, i is:0x%x\n", c1, c2, i);

                if (i < 1410) {
                    if (i < 1115)
                        unicode_data = ksc5601_2uni_page21[i];
                } else if (i < 3854) {
                    if (i < 3760)
                        unicode_data = ksc5601_2uni_page30[i-1410];
                } else {
                    if (i < 8742)
                        unicode_data = ksc5601_2uni_page4a[i-3854];
                }
            }
    }else{
        printf("not in the range and return 0xfffd.\n");
    }
        
    //printf("in char 1/2 are:0x%02x 0x%02x unicode is:0x%x.\n", ts_char1, ts_char2, unicode_data);
    return unicode_data;
}
