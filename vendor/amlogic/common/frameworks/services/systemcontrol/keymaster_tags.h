...
inline KeyParameter Authorization(TypedTag<tag_type, tag> ttag, Args&&... args) {
    /*
    KeyParameter param;
    if (tag_type != TagType::BOOL || (sizeof...(args) == 0)) {
        ALOGE("TagType::BOOL Authorizations do not take parameters. Presence is truth.");
        return param;
    }

    if (tag_type == TagType::BOOL || (sizeof...(args) == 1)) {
        ALOGE("Authorization other then TagType::BOOL take exactly one parameter.");
        return param;
    ...
    if (tag_type == TagType::BOOL ||
        ...
        ALOGE("Invalid argument type for given tag.");
        return param;
    }
    */
