...
class FullAppointedWatchCardPresenter extends DvrItemPresenter<Object> {
    private final Drawable mIconDrawable;
    private final String mCardTitleText;
    private final TvClock mTvClock;

    FullAppointedWatchCardPresenter(Context context) {
        super(context);
        mIconDrawable = mContext.getDrawable(R.drawable.dvr_full_schedule);
        mCardTitleText = mContext.getString(R.string.dvr_appoint_program);
    ...
    public void onBindDvrItemViewHolder(DvrItemViewHolder vh, Object o) {
        ...
        List<Program> programs = ProgramDataManager.getAppointedPrograms(mContext);
        int fullDays = 0;
        if (!programs.isEmpty()) {
            fullDays =
    ...
    protected View.OnClickListener onCreateOnClickListener() {
        return new View.OnClickListener() {
            ...
            public void onClick(View view) {
                DvrUiHelper.startAppointWatchActivity(mContext);
            }
        };
    }
}
