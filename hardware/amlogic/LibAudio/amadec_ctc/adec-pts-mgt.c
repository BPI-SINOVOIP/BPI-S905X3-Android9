...
/**
* \brief before audio start, let the video start
* \param audec pointer to audec
* \return 0 on success otherwise -1
*
*  For network play, sometimes when APTS > VPTS
*  and video data awalys not enough(video will not
*  be quickly play until synchronization). We should
*  be make video play first, until the vpts>=apts
*  resynchronization output
*/
int adec_switch_pts(aml_audio_dec_t *audec, unsigned long firstapts)
{
    ...
    if (property_get("media.vmx.iptv.drm", value, NULL) > 0) {
        ctc_drm = atoi(value);
    }
    if (0 == audec->has_video && ctc_drm == 0) {
        adec_print("has no video\n");
        return 0;
    }
    adec_print("adec_switch_pts start\n");
    do {
    ...
    adec_print("[%s::%d] first_apts 0x%lx, first_vpts 0x%lx diff:0x%lx\n", __FUNCTION__, __LINE__, first_apts, first_vpts, diff);
...
int adec_pts_start(aml_audio_dec_t *audec)
{
    ...
    if (audec->format == ACODEC_FMT_RAAC ||
        audec->format == ACODEC_FMT_WMAPRO ||
        audec->format == ACODEC_FMT_MPEG)
        if (property_get("media.sys.ats.flush", value, NULL) > 0) {
            cts_running = atoi(value);
        }
    if (cts_running == 0) {
