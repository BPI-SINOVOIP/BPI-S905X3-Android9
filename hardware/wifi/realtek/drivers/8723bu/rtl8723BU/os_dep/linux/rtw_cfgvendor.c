		...
		struct wireless_dev *wdev, const void  *data, int len)
{
	...
	output = rtw_malloc(sizeof(wifi_radio_stat_internal) + sizeof(wifi_iface_stat)+1);
	if (output == NULL) {
	...
	err =  rtw_cfgvendor_send_cmd_reply(wiphy, wdev_to_ndev(wdev), 
		output, sizeof(wifi_iface_stat) + sizeof(wifi_radio_stat_internal)+1);
	if (unlikely(err))
		RTW_ERR(FUNC_NDEV_FMT"Vendor Command reply failed ret:%d\n"
			, FUNC_NDEV_ARG(wdev_to_ndev(wdev)), err);
	rtw_mfree(output, sizeof(wifi_iface_stat) + sizeof(wifi_radio_stat_internal)+1);
	return err;
}
