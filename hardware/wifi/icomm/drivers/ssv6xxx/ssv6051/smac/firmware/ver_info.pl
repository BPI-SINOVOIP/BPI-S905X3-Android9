...
use Sys::Hostname;
my $host = hostname();
printf("#define FIRMWARE_COMPILERHOST \"$host\"\n");

use POSIX qw(strftime);
my $date = strftime "%m-%d-%Y-%H:%M:%S", localtime;
printf("#define FIRMWARE_COMPILERDATE \"$date\"\n");
##

use Config;
printf("#define FIRMWARE_COMPILEROS \"$Config{osname}\"\n");
printf("#define FIRMWARE_COMPILEROSARCH \"$Config{archname}\"\n");
